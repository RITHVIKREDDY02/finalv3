...
export default function TrxWingo1Min() {
  ...
  const { data: prediction } = useQuery<TrxWingoPrediction>({
    queryKey: [`/api/trx-wingo/prediction/1min`],
  ...
  const { data: results = [] } = useQuery<TrxWingoResult[]>({
    queryKey: [`/api/trx-wingo/results/1min`],
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: [`/api/trx-wingo/prediction/1min`] });
    queryClient.invalidateQueries({ queryKey: [`/api/trx-wingo/results/1min`] });
  };
