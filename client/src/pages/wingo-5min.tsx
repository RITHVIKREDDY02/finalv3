...
export default function Wingo5Min() {
  ...
  const { data: history = [] } = useQuery<any[]>({
    queryKey: [`/api/wingo/history/5min`],
  ...
  const clearHistoryMutation = useMutation({
    mutationFn: async () => {
      return apiRequest('/api/wingo/history/5min', 'DELETE');
    },
    onSuccess: () => {
      // Immediately invalidate and refetch the history query
      queryClient.invalidateQueries({ queryKey: ['/api/wingo/history/5min'] });
      queryClient.refetchQueries({ queryKey: ['/api/wingo/history/5min'] });
